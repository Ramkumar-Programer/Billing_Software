...
const {insertOpt, selectEmailOpt, deleteOptQueryAuto,deleteOptQuery, selectVerifedOpt, modifyOtpCheck} = require('../../Queries/OTPQuery');
 ...
 async function deleteOtp(type, mail) {
  try {
    if (type === "auto") {
      ...
      {
        //console.log(result[i].email_id)
        await executeQuery(dbConfig, deleteOptQueryAuto, result[i].email_id);
      }
    } else {
